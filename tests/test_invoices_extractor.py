...
5d65feea-e140-496c-927d-085975a492a5,c4432b21-b248-4bff-ab46-05ec06a22da1,2025-07-31T12:00:44.246835Z,OUTFLOW,G03,Vigent,"{}",3.3,"[]",HEAA771128VCX,10516.94,EEM0507226F7,Emisor 97D8,2025-07-31T12:00:02.011094Z,2024-01-21,Ingreso,99,66821.84,1,Receptor 1BAE,PPD,11000,9426.0,"[]",65730.9,2025-07-13T08:50:14.313874,be296788-b2f2,fe5865f14f5fc"""  # noqa: E501
    ...
    def test_extract_method(self, mock_read_csv, extractor, sample_dataframe):
        ...
        days = sample_dataframe[sample_dataframe["type"] == InvoiceType.INFLOW.value][
            "invoice_date"
        ].unique()
        output_days = result["date"].to_list()
        for day in days:
            assert day in output_days

        assert len(output_days) == len(days)
    ...
    @pytest.mark.unit
    @patch("pandas.read_csv")
    def test_extract_outflow_filtering(
        self, mock_read_csv, extractor, sample_dataframe
    ):
    ...
    def test_get_invoices_method(self, mock_read_csv, extractor, sample_dataframe):
        ...
        # Verify all results are INFLOW type
        if len(result) > 0:
            assert all(result["type"] == InvoiceType.INFLOW.value)
    ...
    @pytest.mark.unit
    @patch("pandas.read_csv")
    def test_extract_with_narrow_date_range(
        self, mock_read_csv, extractor, sample_dataframe
    ):
