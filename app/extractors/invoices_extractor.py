...
class InvoicesExtractor:
    ...
    def extract(
        self, from_date: str, to_date: str, invoice_type: InvoiceType
    ) -> pd.DataFrame:
        data = pd.read_csv("./data/invoices.csv")
        data = self._filter_data(data, from_date, to_date, invoice_type)
        data = data.groupby(["invoice_date", "invoice_type"], as_index=False)[
            "total_amount"
        ].sum()
    ...
    def _filter_data(
        ...
        data = data[
            (data["invoice_date"] >= pd.to_datetime(from_date).date())
            & (data["invoice_date"] <= pd.to_datetime(to_date).date())
            & (data["type"] == invoice_type.value)
        ]
        return data
